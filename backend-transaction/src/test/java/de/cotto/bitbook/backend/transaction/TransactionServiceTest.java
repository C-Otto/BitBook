...
class TransactionServiceTest {
    ...
    void downloads_transaction_details_with_standard_priority() {
        when(prioritizingTransactionProvider.getTransaction(requestFor(TRANSACTION_HASH))).thenReturn(TRANSACTION);
    ...
    void downloads_transaction_details_with_standard_priority_by_hashes() {
        when(prioritizingTransactionProvider.getTransaction(requestFor(TRANSACTION_HASH))).thenReturn(TRANSACTION);
        when(prioritizingTransactionProvider.getTransaction(requestFor(TRANSACTION_HASH_2))).thenReturn(TRANSACTION_2);
        when(blockHeightService.getBlockHeight()).thenReturn(BLOCK_COUNT_IN_CHAIN);
    ...
    void requests_price_for_transaction_timestamp() {
        mockResult(TRANSACTION_HASH, TRANSACTION, TRANSACTION);
        when(blockHeightService.getBlockHeight()).thenReturn(BLOCK_COUNT_IN_CHAIN);

        Transaction transaction = transactionService.getTransactionDetails(TRANSACTION_HASH, LOWEST);

        verify(priceService, atLeastOnce()).requestPriceInBackground(transaction.getTime());
    }

    @Test
    void requests_price_for_transaction_timestamp_lowest_priority_request() {
        mockResult(TRANSACTION_HASH, Transaction.UNKNOWN, TRANSACTION);
        when(blockHeightService.getBlockHeight()).thenReturn(BLOCK_COUNT_IN_CHAIN);

        transactionService.getTransactionDetails(TRANSACTION_HASH, LOWEST);
    ...
    void no_price_trigger_for_unknown_transaction() {
        mockResult(TRANSACTION_HASH, Transaction.UNKNOWN, Transaction.UNKNOWN);
    ...
    void does_not_persist_download_for_unconfirmed_transaction() {
        ...
        mockResult(TRANSACTION_HASH, downloadedTransaction, downloadedTransaction);
    ...
    void request_in_background_uses_lowest_priority() {
        ...
        mockResult(TRANSACTION_HASH, TRANSACTION, TRANSACTION);
        mockResult(anotherHash, Transaction.UNKNOWN, Transaction.UNKNOWN);
    ...
    private void mockResult(String transactionHash, Transaction returnedResult, Transaction futureResult) {
        when(prioritizingTransactionProvider.getTransaction(requestFor(transactionHash))).then(invocation -> {
            TransactionRequest transactionRequest = invocation.getArgument(0);
            transactionRequest.getWithResultFuture().provideResult(futureResult);
