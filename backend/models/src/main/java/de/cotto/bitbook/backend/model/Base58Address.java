...
public class Base58Address {
    ...
    public static String createP2Pkh(HexString hash) {
        return createForPrefix(P2PKH_PREFIX, hash);
    }

    public static String createP2Sh(HexString hash) {
        return createForPrefix(P2SH_PREFIX, hash);
    }
    ...
    private static String createForPrefix(HexString prefix, HexString hash) {
        HexString data = prefix.append(hash);
        HexString checksum = data.getSha256Hash().getSha256Hash().getSubstringUpToByte(4);
        HexString dataToEncode = data.append(checksum);
        return encode(dataToEncode);
    }

    private static String encode(HexString input) {
        return new Base58Encoder(input).encode();
    }
