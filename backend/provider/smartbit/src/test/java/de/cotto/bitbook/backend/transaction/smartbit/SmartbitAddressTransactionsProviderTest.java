...
import static de.cotto.bitbook.backend.transaction.smartbit.SmartbitAddressTransactionDtoFixtures.SMARTBIT_TRANSACTIONS_UPDATED;
import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.Mockito.when;

@ExtendWith(MockitoExtension.class)
class SmartbitAddressTransactionsProviderTest {

    @InjectMocks
    private SmartbitAddressTransactionsProvider smartbitAddressTransactionsProvider;
    ...
    void getAddressTransactions() {
        when(smartbitClient.getAddressTransactions(ADDRESS)).thenReturn(Optional.of(SMARTBIT_ADDRESS_TRANSACTIONS));
        Optional<AddressTransactions> addressTransactions = smartbitAddressTransactionsProvider.get(
    ...
    @Test
    void getUpdates() {
        when(smartbitClient.getAddressTransactions(ADDRESS)).thenReturn(Optional.of(SMARTBIT_TRANSACTIONS_UPDATED));
        ...
        Optional<AddressTransactions> updated =
                smartbitAddressTransactionsProvider.get(transactionsRequestKey);
        assertThat(updated).contains(ADDRESS_TRANSACTIONS_UPDATED);
    }

    @Test
    void getUpdates_no_update_returned() {
        when(smartbitClient.getAddressTransactions(ADDRESS)).thenReturn(Optional.empty());

        Optional<AddressTransactions> updated = smartbitAddressTransactionsProvider.get(
    ...
    @Test
    void getName() {
        assertThat(smartbitAddressTransactionsProvider.getName()).isEqualTo("SmartbitAddressTransactionsProvider");
    }
}